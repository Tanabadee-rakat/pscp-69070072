...
```text
โจทย์ให้รับค่าคะแนนรวมของทั้ง 3 คน และค่าคะแนนสูงสุดของทั้งสามคน เพื่อนำมาหาว่าคะแนนสูงสุดและคะแนนต่ำสุดห่างกันเกิน 2 คะแนนไหม โดยคะแนนเป็นไปได้ตั้งแต่ 0-10 และไม่จำกัดว่าต้องเป็นจำนวนเต็ม

Input:
โปรแกรมรับค่าคะแนนรวมของทั้ง 3 คนและค่าคะแนนสูงสุด

Output:
โปรแกรมควรพิมพ์คำว่า "Not surprising" ถ้าคะแนนห่างกันไม่เกิน 2 คะแนนและ "Surprising" ถ้าคะแนนห่างกันเกิน 2 คะแนน

Constraints:
input ไม่จำกัดว่าเป็นจำนวนเต็มจึงต้องรับเข้ามาเป็น float ไว้ก่อน
```
...
```text
Step 1:รับคะแนนรวมกับค่าสูงสุด
Step 2:กำหนดตัวแปรค่าสูงสุดอันดับ 2 และสุดท้าย
Step 3: if ตรวจว่าคะแนนรวมมากกว่าหรือเท่ากับค่าสูงสุด
Step 4:ถ้าจริงให้ค่าอันดับสองเท่ากับค่าสูงสุดและกำหนดให้ค่าสุดท้ายเท่ากับค่าอันดับสองลบด้วยคะแนนรวม
Step 5: elif ตรวจว่าคะแนนรวมน้อยกว่าค่าสูงสุด
Step 6:ถ้าจริงให้ค่าอันดับสองเท่ากับคะแนนรวมและกำหนดให้ค่าสุดท้ายเท่ากับ 0
Step 7: if ตรวจสอบว่า ค่าสูงสุดลบค่าน้อยสุดเกิน 2 หรือไม่
Step 8: ถ้าจริงให้พิมพ์ "Surprising"
Step 9: ถ้าไม่จริงให้พิมพ์ "Not surprising"
```
...
```text
วิธีสุดท้ายฉันเหมือนกับแผนแรก ฉันกำหนดค่าอันดับ 2 และ 3 มาก่อน แล้วค่อยเอาค่าสูงสุดลบออกจากคะแนนรวม หลังจากนั้นค่อยเปรียบเทียบค่าที่เหลือว่าอันดับ 2 จะมีค่าที่สูงที่สุดได้ค่าใด แล้วค่อยกำหนดค่าอันดับ 3 เท่ากับคะแนนรวมที่ถูกลบด้วยอันดับ 1 และ 2 แล้วจึงค่อยนำค่าสูงสุดและต่ำสุดมาเปรียบเทียบกัน
```
...
```text
เพื่อตรวจสอบในกรณีที่ Input เป็น float
```
...
```text
เพื่อตรวจสอบว่าการคำนวณผิดพลาดหรือไม่
```
...
```text
เพิ่อตรวจสอบว่าถ้าค่าสูงสุดมีค่าเท่ากับคะแนนรวมจะเกิด Bug หรือไม่
```
