...
```text
โจทย์ให้รับค่า 4 ค่า โดยมี A B d และ r ให้ A - B เป็นช่วงของจำนวนนับ ต้องการรู้ว่ามีกี่จำนวนในช่วงที่หารด้วยจำนวนนับ d แล้วได้เศษ r

Input:
โปรแกรมรับจำนวนนับ 4 จำนวน A, B, d และ r

Output:
แสดงจำนวนของจำนวนนับที่อยู่ในช่วง A-B และหารด้วย d ได้เศษ r

Constraints:
input เป็นจำนวนนับเท่านั้นแปลว่าฉันไม่ต้องทำเงื่อนไขที่เป็น 0
```
...
```text
Step 1: รับค่าจำนวนนับเป็นตัวแปรทั้ง a b d r
Step 2: กำหนดค่า count = 0
Step 3: ให้ while ขณะที่ค่า b >= a
Step 4: ถ้า b >= a ให้เช็คด้วย if ว่า b % d มีค่าเป็น r หรือไม่ (% คือหารเอาเศษ)
Step 5: ถ้า if เป็นจริงให้ count += 1
Step 6: นำค่า b มา -= 1 เพื่อไปดูจำนวนน้อยกว่าจนกว่าจะน้อยกว่า a
Step 7: ถ้า b ไม่มากกว่าเท่ากับ a แล้ว ให้แสดงค่า count
```
...
```text
วิธีสุดท้ายของฉันใช้แบบแผนแรก ฉันรับค่าตัวแปรมาเสร็จ กำหนดค่า count ที่เอาไว้นับจำนวน แล้วก็สร้าง loop while เพื่อเช็คว่าจำนวนตั้งแต่ a-b มีกี่ตัวที่หารเอาเศษ ด้วย d แล้วได้เศษ r โดยใช้ตัวแปร b ค่อยๆลบออกที่ละ 1 จนกว่าจะหลุดระยะ (น้อยกว่า a) ถ้าเลขที่เช็คเข้าเงื่อนไข หารด้วย d ได้เศษ r ก็จะ count += 1 แล้วจึงแสดงค่า count หลังจากหลุดออกจาก while loop
```
...
```text
ใช้เช็คเพื่อหา error ในกรณีที่ a กับ b เท่ากัน
```
...
```text
ใช้เช็ค while ในกรณีที่ a ไม่ได้เริ่มจาก 1
```
...
```text
ใช้เช็คเงื่อนไขของ mod เพราะว่าถ้าหารด้วย 1 แล้วได้เศษ 1 จะไม่มีเลขได้เข้าเงื่อนไขทั้งสิ้น
```
