...
```text
โจทย์ให้ฉันรับสีมาสองค่า แล้วนำมารวมกันตามแม่สีที่ให้ โดยกำหนดมาให้หมดแล้วว่าสีอะไรรวมกับสีอะไรได้สีอะไร ให้ตอบเป็นสีนั้นใน 1 บรรทัด

Input:
รับสีที่ 1 กับสีที่ 2 คนละบรรทัด

Output:
โปรแกรมแสดงค่าสีที่ผสมได้โดยถ้าคู่สีไม่ตรงกับค่าใดที่กำหนดให้แสดง Error แทนชื่อสี

Constraints:
ต้องเชคชื่อสีว่าตรงกับที่กำหนดไว้หรือไม่ พิจารณา testcase ที่เป็นไปได้ ควรใช้ .lower() เพื่อให้มั่นใจว่าไม่มีการสะกดที่ไม่ตรงกับเงื่อนไข
```
...
```text
Step 1:รับชื่อสีทั้ง 2
Step 2:ใช้ .lower() กับทั้งสอง เพื่อให้มั่นใจว่าผู้ใช้เขียนชื่อถูก
Step 3:ใช้ if กับ elif สร้างเงื่อนไขว่าสีใดผสมกับสีอะไร ต้อง พิมพ์เป็นสีไหน
Step 4:ใช้ else ในการเก็บตกสีที่ไม่ถูกกำหนดในโจทย์ และพิมพ์ Error
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกเกือบหมด แต่ฉันได้เพิ่มเงื่อนไขสำหรับสีที่ซ้ำกันผสมกันขึ้นมาเนื่องจากในตอนแรกฉันไม่ได้ป้องกัน testcase ของสีที่ซ้ำกันเอาไว้ ทำให้ติด Testcase บางส่วน
```
...
```text
เพื่อเช็คเงื่อนไขที่ทำให้ Error
```
...
```text
เพื่อเช็คว่าใช้งาน .lower() ได้ถูกต้องหรือไม่
```
...
```text
เพื่อป้องกันในกรณีที่สีซ้ำกันและไม่ได้เกิดสีใหม่
```
