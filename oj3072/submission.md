...
```text
โจทย์ให้ฉันนับสระที่อยู่ในข้อความที่มอบให้ และแสดงผลลัพธ์

Input:
โปรแกรมรับข้อความเป็น string

Output:
แสดงจำนวนสระที่นับได้

Constraints:
input เป็นจำนวน string โดยสระตัวใหญ่หรือตัวเล็กถือว่าเป็นตัวเดียวกัน
```
...
```text
Step 1:รับข้อความเป็น string พร้อมใช้ .lower() เพื่อให้นับได้ครบทุกตัว
Step 2:แปลงข้อความให้เป็น list
Step 3:ใช้ if ตรวจแต่ละสระโดยใช้ .count(" ") ถ้า .count(" ") ไม่ใช่ 0 จะพิมพ์ข้อความแสดงจำนวนของสระนั้นๆ ทำให้ครบ 5 if, a e i o u
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ .lower() แปลงหลังจากรับข้อความมา และ list() เพื่อแยกตัวอักษร หลังจากนั้นก็สร้าง if ที่ count ตัวสระแต่ละตัวและพิมพ์ออกมา ทั้งหมด 5 if
```
...
```text
เพื่อเช็คว่ากลไก if ทำงานถูกต้อง โดยจะต้องไม่ตรงเงื่อนไขอะไรเลย
```
...
```text
เพื่อเช็คว่า .lower() ถูกเรียกใช้อย่างถูกต้อง
```
