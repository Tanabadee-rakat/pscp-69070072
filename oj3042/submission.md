...
```text
โจทย์รับข้อมูลจำนวนเต็มบวก ให้ปัดเป็นเลขที่ 10 หารลงตัว(ปัดลง)และพิมพ์เลขที่ค่อยๆลดลงทีละ 0 ตั้งแต่ตัวก่อนลบจนถึง 0

Input:
โปรแกรมรับค่าจำนวนเต็มบวก

Output:
แสดงจำนวนเต็มบวกตั้งแต่ตัว Input จนถึง 0 ที่หารด้วย 10 ลงตัว

Constraints:
input เป็นจำนวนเต็มบวกที่อาจจะไม่ได้ลงท้ายด้วย 0 ฉันควรจะทำให้เลข Input เป็นเลขที่ 10 หารลงตัวก่อน
```
...
```text
Step 1:รับค่าจำนวนเต็มบวกมาเป็น String
Step 2:สร้างตัวแปรที่เอาตัวก่อนหน้า index[-1] ของ Input มาต่อกับ 0 เพื่อให้หาร 10 ลงตัว
Step 3:แปลงชนิดตัวแปรกลับเป็น int
Step 4:ใช้ลูป while ถ้า Input ไม่ใช่ 0 ให้พิมพ์ Input โดยใช้ end=" " เพื่อให้ไม่เว้นบรรทัด
Step 5:ลบค่าออกจากตัวแปร Input ทีละ 10
Step 6:หลุดลูป while มาแล้ว พิมพ์ 0 ปิดท้าย เพราะ while หยุดทำงานตอน Input เป็น 0
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันเราจำนวนเต็มบวกเข้ามา เปลี่ยนหลักท้ายให้เป็นศูนย์ก่อน แปลงค่ากลับเป็น int แล้วจึงนำไปเข้าลูป while ที่จะพิมพ์ค่า Input ก่อนจะลบไปสิบเพื่อไปเริ่มลูปใหม่ หลังจากหลุดลูปมาก็พิมพ์ 0 ปิดท้าย Output
```
...
```text
เพื่อทดสอบว่าสามารถปัดเศษที่หารด้วย 10 ไม่ลงตัวออกได้ถูกต้อง
```
...
```text
เพื่อป้องกัน testcase ว่าถึง Input จะเป็น 0 ก็ต้องพิมพ์ 0 อยู่ดี
```
...
```text
เพื่อลองการปัดเศษลงในหลักเดียว
```
